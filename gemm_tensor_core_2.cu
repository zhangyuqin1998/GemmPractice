...
template<class SrcT>
__device__ __inline__ void ldmatrix_sync_x4_m8n8(
    uint32_t &r0, uint32_t &r1, uint32_t &r2, uint32_t &r3, SrcT addr
) {
...
__device__ __inline__ void mma16x16x16(
    FragmentA frag_a, FragmentB frag_b, FragmentC frag_c)
{
#pragma unroll
    for (int i = 0; i < 2; ++i) {
...
__global__ void Kernel(const half *A, const half *B, float *C, uint64_t m, uint64_t n, uint64_t k) {
    ...
    uint64_t st_seizzled_offset = Swizzle<2, 3, 3>::apply(k_in_tile, mn_in_tile, TILE_SIZE, TILE_SIZE);
    uint64_t ld_swizzled_offset = Swizzle<2, 3, 3>::apply(laneid / 16 * 8, laneid % 16, TILE_SIZE, TILE_SIZE);

    for (uint64_t k_tile = 0; k_tile < k; k_tile += TILE_SIZE) {
        uint64_t k_idx = k_tile + k_in_tile;

        auto a_src_addr = &A[(m_idx) * k + k_idx];
        uint32_t sa_dst_addr = __cvta_generic_to_shared(&s_A[st_seizzled_offset / TILE_SIZE][st_seizzled_offset % TILE_SIZE]);
        cp_async_128B(sa_dst_addr, a_src_addr);

        auto b_src_addr = &B[(n_idx) * k + k_idx];
        uint32_t sb_dst_addr = __cvta_generic_to_shared(&s_B[st_seizzled_offset / TILE_SIZE][st_seizzled_offset % TILE_SIZE]);
        ...
        uint64_t ldmatrix_sa_addr = __cvta_generic_to_shared(&s_A[ld_swizzled_offset / TILE_SIZE][ld_swizzled_offset % TILE_SIZE]);
        ldmatrix_sync_x4_m8n8(
            frag_a[0], frag_a[1], frag_a[2], frag_a[3],
            ldmatrix_sa_addr
        );

        uint64_t ldmatrix_sb_addr = __cvta_generic_to_shared(&s_B[ld_swizzled_offset / TILE_SIZE][ld_swizzled_offset % TILE_SIZE]);
        ldmatrix_sync_x4_m8n8(
            frag_b[0][0], frag_b[1][0], frag_b[0][1], frag_b[1][1],
            ldmatrix_sb_addr
        );
