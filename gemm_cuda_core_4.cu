...
__global__ void Kernel(const half *A, const half *B, float *C, uint64_t m,
  ...
  for (uint64_t k_tile = 0; k_tile < k; k_tile += TILE_SIZE) {
    if (threadIdx.x < TILE_SIZE / 8) {
      uint64_t new_offset = Swizzle<2, 3, 3>::apply(
          threadIdx.x * 8, threadIdx.y, TILE_SIZE, TILE_SIZE);
      LDVectorized<float4>(
          &(s_A[new_offset / TILE_SIZE][new_offset % TILE_SIZE]),
    ...
    if (threadIdx.x < TILE_SIZE / 8) {
      uint64_t new_offset = Swizzle<2, 3, 3>::apply(
          threadIdx.x * 8, threadIdx.y, TILE_SIZE, TILE_SIZE);
      LDVectorized<float4>(
          &(s_B[new_offset / TILE_SIZE][new_offset % TILE_SIZE]),
          &(B[(blockIdx.x * blockDim.x + threadIdx.y) * k + k_tile +
              threadIdx.x * 8]));
    }
    __syncthreads();
...
#pragma unroll
      for (uint64_t t = 0; t < TILE_SIZE; t += 8) {
        half f_A[8];
        half f_B[8];
        uint64_t new_offset_sa =
            Swizzle<2, 3, 3>::apply(t, threadIdx.y, TILE_SIZE, TILE_SIZE);
        uint64_t new_offset_sb =
            Swizzle<2, 3, 3>::apply(t, threadIdx.x, TILE_SIZE, TILE_SIZE);
        LDVectorized<float4>(
            f_A, &(s_A[new_offset_sa / TILE_SIZE][new_offset_sa % TILE_SIZE]));
        LDVectorized<float4>(
            f_B, &(s_B[new_offset_sb / TILE_SIZE][new_offset_sb % TILE_SIZE]));
#pragma unroll
